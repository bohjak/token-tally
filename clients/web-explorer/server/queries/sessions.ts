...
export function getSession(db: Database.Database, sessionId: string): GetSessionResult | null {
  const session = db
    .prepare(
      `SELECT
         s.id, s.harness_id, s.cwd, s.repo_owner, s.repo_name, s.repo_remote,
...
export function getTurnDetail(db: Database.Database, turnId: string): GetTurnDetailResult | null {
  const turn = db
    .prepare(
      `SELECT
         t.id, t.turn_index, t.started_at, t.ended_at, t.model_id,
