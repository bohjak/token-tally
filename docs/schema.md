...
Writers upsert this row at the start of each session with `INSERT OR REPLACE`.
...
**Performance budget:** today/week summary and per-harness breakdown queries on a 1M-row `llm_messages` fixture must complete in under 50 ms on a modern Mac, exercised by `AnalyticsTrayTests/PerformanceTests.swift`. If indexes alone cannot meet this budget, the next step is a `daily_usage` rollup table populated by writers or a `token-tally rollup` command — not heavier indexes.
...
| `subscriptions` | `(harness_id, plan_name, period_start)` | `DO UPDATE` (update quota_used, period_end) |
